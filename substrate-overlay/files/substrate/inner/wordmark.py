"""The Substrate brand wordmark and Otto lockup for CLI output.

A bold "ANSI-Shadow" block-letter ``substrate`` wordmark — the canonical
...
from :mod:`substrate.inner.mascots`, which it pairs with 1:1.
...
banner (TTY gating, ``SUBSTRATE_NO_BANNER``) lives one layer up in
:mod:`substrate.inner.ui`, which is the only module that should be imported
...
:data:`substrate.inner.mascots.MASCOT_ART_COLOR`); the optional gradient
...
#: The rows of the ``substrate`` wordmark, as plain (uncolored) text.
...
def render_lockup(
    ...
    *,
    ...
    :param epilogue: Optional aligned label/value rows printed beneath the
        art, e.g. ``[("Version", "0.4.2"), ("Next", "substrate setup")]``.
    :returns: None.
    """
...
def render_compact(console: Console, *, subtitle: str | None = None) -> None:
    """
    Print the one-line brandmark: ``✦ substrate  <subtitle>``.
    ...
    line.append("substrate", style=f"bold {WORDMARK_COLOR}")
    if subtitle:
